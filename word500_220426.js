...
function initialize() {
	...
	if (lang == null) {
		...
		word = pu(up('word')); //Process URL - read URL Params
	}
	rg(); //ResetGame
...
function rw(iRow) { //Read word from current line
    let sWord = '';
    for (let i = 0; i < width; i++) {
        let currTile = document.getElementById(iRow.toString() + '-' + i.toString());
...
var up = function (name, w) { // URL Parameters
    w = w || window;
